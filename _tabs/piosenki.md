...
<div id="desert-box" 
  ...
  <div class="desert-overlay">
    ...
    <p style="color: white;">Nie znaleźliśmy takiej piosenki.</p>
    <button id="clear-search-btn" style="margin-top: 15px; padding: 10px 30px; border-radius: 20px; border: none; background: #007bff; color: white; cursor: pointer; font-weight: bold;">Wyczyść</button>
  </div>
</div>
  ...
  function applyFilter() {
    ...
    heads.forEach(h => {
      ...
      while (next && next.classList.contains('s-item')) {
        if (!next.classList.contains('s-hide')) { visible = true; break; }
        next = next.nextElementSibling;
      }
      h.classList.toggle('s-hide', !visible);
    });
    ...
    if (val.length > 0) {
      ...
      if (found === 0) {
        ...
        desert.style.backgroundImage = 'url(' + (isDark ? desert.dataset.night : desert.dataset.day) + ')';
      ...
      heads.forEach(h => h.classList.remove('s-header-hide')); // Fix dla liter
    }
  }

  // "ZEGAROWY STRAŻNIK" - co 500ms sprawdza czy szukanie działa
  setInterval(() => {
    const input = document.getElementById('song-search');
    if (input && !input.dataset.active) {
      input.dataset.active = "true";
      input.addEventListener('input', applyFilter);
      console.log("Wyszukiwarka aktywowana!");
    }
